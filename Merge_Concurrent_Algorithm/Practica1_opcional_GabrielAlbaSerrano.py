...
N = 7 #cantidad de números que producen los productores
K = 5 #capacidad de los buffer
...
def add_data(storage, index, pid, data, mutex):
    ...
    # Si el valor del índice del respectivo almacén en el que queremos producir un número
    # no es menor que la capacidad del buffer, esperamos con un bucle, hasta que sea menor,
    # en cuyo caso producimos el número 'data', añadiéndolo en la posición que marca
    # 'index.value' en el almacén 'storage'
    if not index.value < K:
        while not index.value < K:
            pass
    else:
        mutex.acquire()
        try:
...
def producer(storage, index, empty, non_empty, mutex):
    ...
    # Añadimos cada número en el almacén correspondiente
    for n in nums:
